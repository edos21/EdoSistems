<?php
	
	include 'includes/header.html';

?>
	
	<div id="wrapper">
		<div id="contenedor3dimage">
			<section id="dg-container" class="dg-container">
				<div class="dg-wrapper">
					<a href="#"><img src="img/slider/1.jpg" alt="image01"><div></div></a>
					<a href="#"><img src="img/slider/2.jpg" alt="image02"><div></div></a>
					<a href="#"><img src="img/slider/3.jpg" alt="image03"><div></div></a>
					<a href="#"><img src="img/slider/4.jpg" alt="image04"><div></div></a>
					<a href="#"><img src="img/slider/5.jpg" alt="image05"><div></div></a>
				<!--	<a href="#"><img src="img/slider/6.jpg" alt="image06"><div></div></a>
					<a href="#"><img src="img/slider/7.jpg" alt="image07"><div></div></a>
					<a href="#"><img src="img/slider/8.jpg" alt="image08"><div></div></a>
					<a href="#"><img src="img/slider/9.jpg" alt="image09"><div></div></a>
					<a href="#"><img src="img/slider/10.jpg" alt="image10"><div></div></a>
					<a href="#"><img src="img/slider/11.jpg" alt="image11"><div></div></a>
					<a href="#"><img src	="img/slider/12.jpg" alt="image12"><div></div></a> -->
				</div>
				<br><br>
				<div id="cont1">
				<div id="textizq">DISEÑAMOS Y<br>DESARROLLAMOS</div>
				<div id="textdere">TU PAGINA WEB</div>
				</div>
			</section>
				<img id="sombra" src="img/shadow_1.png"/>
	
		</div>
	<!--AQUI VA EL RESTO DE HOME-->
		<br>
		<div>¡Hola! Bienvenido a Edosistems, una empresa dedica al desarrollo y diseño Web, acá encontrarás toda la información referente a nuestros servicios que ponemos a tu disposición:</div>
		<br>
		Nuestros Servicios
		<br>
<div id="cont2">

	<div id="desarrollo">
		<div id="logito"><img src="img/servi1.png"></div>
		<br>
		<b>Desarrollo</b>
		<br>
		¿Necesitas el desarrollo profesional de tu idea, proyecto, o bien un sitio web personal o corporativo? En Edosistems proyectamos excelencia, por ello te ofrecemos una amplia gama de servicios referentes al desarrollo desde front-end, back-end, desarrollo general, planificación del proyecto y más. Desarrollamos usando las últimas tecnologías y más productivas técnicas.
	</div>

	<div id="asesoria">
		<div id="logito"><img src="img/servi1.png"></div>
		<br>		
		<b>Asesorias</b>
		<br>
		Dos cabezas piensas más que una. Ya seas profesional o estudiante, te ofrecemos personal capacitado para ayudarte en áreas de desarrollo y programación para cualquiera de tus proyectos, con atención individual o por grupos, con los más eficientes lenguajes y tecnologías y en excelentes precios y horarios diseñados para tu mayor comodidad.
	</div>

	<div id="cursos">
		<div id="logito"><img src="img/servi1.png"></div>
		<br>
		<b>Cursos</b>
		<br>
		En Edosistems ofrecemos cursos prácticos y completos de las mejores tecnologías de Desarrollo Web, PHP, MySQL, JavaScript, HTML5 y más. 
		<br>
		Ponemos a tu disposición diferentes horarios y una flexible organización con los mejores precios. Pagos por hora o por curso completo, además de descuentos para grupos.
	</div>
	<div><img id="sombra2"src="img/shadow_2.png"/></div>
</div>

</div>

<?php

	include 'includes/footer.html';

?>

